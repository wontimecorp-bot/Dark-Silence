...
| ADR-0015 | Ship AI architecture — utility-scored state-machine brains with hierarchical command and behavior-LOD | accepted | 2026-06-10 | — | [0015-ship-ai-utility-state-machine-hierarchical-lod.md](adrs/0015-ship-ai-utility-state-machine-hierarchical-lod.md) |
...
- **2026-06-10 (plan E011/00008-ship-ai):** Added **ADR-0015** (ship AI architecture — utility-scored state-machine brains, hierarchical squad/wing/aggregate command, behavior-LOD). Reusable AI baseline for all future NPC/automation work (the technical substrate of ADR-0011's automation floor): AI controls ships **only via `ShipIntent`** (one code path with human pilots); behavior state is an **enum in one brain component** (never per-state marker components — archetype thrash); decisions are **event-driven + stable-id-hash cadence buckets**, strict-f32 utility scoring with seeded (entity,tick) hashing and **entity-id tiebreaks** (the silent replay-breaker); cost scales with **squads, not ships**. The spatial index becomes **tiered build-once-read-many**: the fine collision broadphase + a new **coarse interest tier** (drives AOI/sim-LOD off authoritative player positions — never a per-client camera — and seeds ADR-0005/0006 sector-sharding). Perception is **sensor/signature-gated at every tier** with a per-faction **datalink fusion** over connected components (transmit-member flood-fill; per-ship jammed/severed **seam flag** — the EW mechanic + Sensor/Datalink module taxonomy is CAP-007). AI state is **ephemeral/re-derivable** (not serialized; persistence epic later). Perf gate: AI overhead ≤ ~30% of the no-AI `fleet_stress` baseline tick.
- **2026-06-04 (specify E004):** The **persistence substrate** (realizes ADR-0007's PostgreSQL+Redis pick + the `persistence` crate boundary), reusable by every durable epic (E005 shared world, E013 economy, E016 research, E017 reputation): a new **`crates/persistence`** exposes **repository abstractions** over **PostgreSQL** (durable source of truth) + **Redis** (hot cache, never sole truth) via **`sqlx`** (compile-time-checked queries + checked-in offline metadata so CI needs no live DB) with **versioned `sqlx` migrations**, versioned persisted formats, and backup-before-migrate. **Accounts** carry a stable id (the connect-token client id) + a salted **Argon2id** credential hash (never plaintext/logged; minimal personal data per the 13+/data-minimization posture). The server gains its **first async/IO**: persistence runs on an **off-thread async runtime reached by non-blocking channels** so a slow/blocked query **never perturbs the deterministic fixed-step tick** — `sim` stays IO-free (no DB/async-runtime dependency), no IO inside fixed-step systems, and the determinism + bot-harness/botkit equivalence stays bit-identical. The **account-backed `TokenIssuer`** replaces E003's `StubTokenIssuer` behind the unchanged `secure_*` path → **authenticated connect** (no token without auth). **PG-down graceful degradation**: serve reads from Redis, queue durable writes for bounded replay, refuse risky state-mutating actions until recovery. The "off-thread persistence IO never blocks the tick" rule is the cross-cutting constraint future server-touching epics (E005/E008/E009/E013) inherit.
