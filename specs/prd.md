...
- **2026-06-04 (E004 spec):** Persistence/account substrate for CAP-002 (shared persistent universe) — durable PostgreSQL + hot Redis storage plus player **accounts** with securely-hashed credentials (Argon2id; minimal personal data consistent with the 13+/data-minimization posture). Connecting to the world now requires an authenticated account (an account-backed connect-token issuer replaces the prior stub), reinforcing the anti-cheat/anti-RMT posture — identity is real, and no real-money or power path is introduced (never pay-to-win preserved). The persistence layer is the durable foundation E005 (shared world) and later durable systems (economy, research, reputation) build on. Source: `specs/00004-persistence-layer/spec.md`.
