...
- **Secure transport**: sessions use an authenticated + encrypted channel (the netcode adapter's secure mode, connect-token connections; the concrete library is named in Constraints/ADR-0014); an unauthenticated/unsecure connect is rejected. The connect-token **issuer** is a stub (local signing key) in E003 — E004 replaces it with account-backed issuance; the secure-connection wiring is reused.
...
- **Library isolated** behind the `protocol` crate + adapter (ADR-0007/ADR-0014); no netcode-library type in gameplay/`sim`/protocol-consumer surfaces (swappable). The concrete netcode library is **renet** (ADR-0014, refining ADR-0007's earlier lightyear pick), with `bevy_replicon`/`aeronet` as fallbacks.
...
- **IP-004**: Uses the netcode library chosen in ADR-0014 (**renet**, refining ADR-0007), confined to the adapter; the transport is UDP.
...
- **TR-011**: The server MUST validate inputs — clamping thrust/turn to physical bounds and rate-limiting fire — and MUST reject impossible, replayed, or stale inputs without affecting authoritative state. (This umbrella requirement is decomposed per-clause by TR-020 (per-field clamp/reject), TR-021 (fire rate), and TR-022 (replay/stale); the enumerated rejection tests are TR-038.)
...
- **TR-013**: Snapshots MUST be delta-encoded against each client's last-acknowledged snapshot, with quantized fields, sent at 15–20 Hz (baseline default 20 Hz, TR-044; below the 30 Hz tick).
...
- **TR-034** *(the loopback test that asserts TR-016 / TR-032(a)'s bit-identical guarantee)*: A determinism test MUST drive both the authoritative server `sim` and the client predicted `sim` from a **known, fixed initial state (seed) and an identical numbered `ClientInput` stream** (the same `seq`-ordered inputs fed to both), step both for a fixed number of ticks in the deterministic loopback harness, and assert the two resulting `sim` states are **bit-identical** (SC-007). The test inputs (seed, initial entity set, the numbered input stream, and tick count) MUST be fixed so the comparison is reproducible run-to-run.
...
- **TR-048**: Sessions MUST be established over an **authenticated + encrypted** transport (the netcode adapter's secure mode, connect-token connections; the concrete library is named in Constraints/ADR-0014); an unauthenticated/unsecure connect attempt MUST be **rejected**. The connect-token **issuer** is a stub (local signing key) in E003, isolated so E004 replaces it with account-backed issuance without touching the secure-connection path; netcode-library specifics stay confined to the netcode adapter module (TR-005). (The netcode.io connect-token scheme is authenticated *and* encrypted together — there is no auth-without-encryption split.)
...
- The netcode library chosen in ADR-0014 (renet; refining ADR-0007) — or its fallback — provides a usable UDP transport behind the adapter.
...
- **SC-010** [OBJ5]: A malformed/undecodable packet, a replayed/stale/out-of-order input, and an input that exceeds the per-client buffer or packet-rate bound are each discarded with the authoritative state unchanged and the offending connection logged, with no unbounded buffer growth (TR-022/023/027/028/030/031). (Distinct from SC-003, which covers the in-bounds invalid-input *classes*; SC-010 adds the malformed-packet / buffer-bound / rate-limit / logging layer.)
...
**Advisory (LOW, non-blocking)**: the body also relies on ADR-0005 / ADR-0006 / ADR-0013 / **ADR-0014 (renet netcode, refining ADR-0007)** beyond the frontmatter's canonical `{ADR-0002}{ADR-0007}` — fold these into plan traceability. TR-013 "quantized fields" is the baseline encoding (the figure E009 optimizes against), distinct from E009's deferred quantization-budget.
