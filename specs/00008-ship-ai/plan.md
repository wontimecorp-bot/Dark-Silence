...
- **Statistic**: overhead = (mean_ai − mean_baseline) / mean_baseline ≤ 30% (hard — >30.0% fails); p50/p99 reported, and p99 must hold the 33.3 ms tick budget so promotion bursts / scan ticks cannot pass on a favorable average.
...
| TR-015 | scenario roles | `crates/server/src/scenario.rs`, `ai/brain.rs` | goal+posture; brain fills tactics |
